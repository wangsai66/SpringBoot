...
public abstract class AbstractUserRealm extends AuthorizingRealm {
     ...
     //获取用户组的权限信息
     public abstract UserRolesAndPermissions doGetGroupAuthorizationInfo(AuthUser userInfo);
     //获取用户角色的权限信息
     public abstract UserRolesAndPermissions doGetRoleAuthorizationInfo(AuthUser userInfo);
	    ...
	    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
	        ...
	        logger.info("===========currentLoginName========="+currentLoginName);
	        //从数据库中获取当前登录用户的详细信息
	        AuthUser userInfo = userService.findByLoginName(currentLoginName);
	        if (null != userInfo) {
	    ...
	    protected AuthenticationInfo doGetAuthenticationInfo(
	        ...
	        //查出是否有此用户
	        AuthUser user = userService.findByLoginName(token.getUsername());
	        if (user != null) {
	            // 若存在，将此用户存放到登录认证info中，无需自己做密码对比，Shiro会为我们进行密码对比校验
	            return new SimpleAuthenticationInfo(user.getLoginName(), user.getUserPassword(), getName());
	        }
	        return null;
	    }
